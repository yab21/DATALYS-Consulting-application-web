...
const ListePartenaires: React.FC = () => {
  ...
  const testImageAvailability = async (url: string): Promise<boolean> => {
    try {
  ...
  const fixImageUrl = (url: string | undefined): string | undefined => {
    ...
    if (url.includes('localhost:8081')) {
      // Extraire le nom du fichier de l'URL localhost
    ...
    // Pour les URLs qui pointent déjà vers 82.112.253.137:8081 mais avec l'ancien format
  ...
  useEffect(() => {
    const loadPartners = async () => {
      ...
      console.log("🔑 Debug état authentification liste:", {
        isAuthenticated,
        authLoading,
                ...
                <ModalHeader className="flex flex-col gap-1 rounded-t-large border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50 pb-6 dark:border-gray-700 dark:from-blue-900/20 dark:to-indigo-900/20">
                  <div className="flex w-full items-center gap-6">
                    <div className="relative flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg dark:border-gray-600 dark:bg-gray-700">
                      {isValidImageUrl(selectedPartner.logo_url) && !imageErrors.has(selectedPartner.id.toString()) ? (
                        <img
                          src={fixImageUrl(selectedPartner.logo_url)!}
                          alt={selectedPartner.name}
                          className="h-full w-full object-cover p-2"
                          onError={() => handleImageError(selectedPartner.id, selectedPartner.logo_url)}
                        />
                      ) : (
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-600 dark:bg-gray-700">
                      ...
                      <div className="flex items-center gap-2">
                        <div className={`h-3 w-3 rounded-full ${selectedPartner.is_active ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
